---
name: deep-research-ab
description: Research a question from the best live evidence — official docs, source, or practitioner communities — read directly and triangulated, fetched where possible and driven in a browser (via the agent-browser CLI skill) when not. A/B variant of deep-research on agent-browser. Type /deep-research-ab.
...
The browser mechanics (the agent-browser skill, the persistent profile, logins, per-platform
quirks, extraction) live in browser.md — read it up front when your plan names a browser
platform (step 1), or whenever a platform later misbehaves.
...
- the **native search you ran** and how many hits you triaged from it, not just the
  ...
  like a human** (type the query and submit) rather than skipping the search when the URL
  won't work;
